...
for f in files:
    ...
    # Step 1: write to temp dir as CSV
    out_df.coalesce(1).write.option("header", True).mode("overwrite").csv(temp_dir)

    # Step 2: find the part file
    part_file = [x.name for x in mssparkutils.fs.ls(temp_dir) if x.name.startswith("part-") and x.name.endswith(".csv")][0]
    ...
    print(f"[SUCCESS] Written single CSV file: {final_path}")
